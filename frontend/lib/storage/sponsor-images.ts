...
function storageClient() {
  ...
  if (!url || !serviceRoleKey) {
    throw new Error(
      "SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must be set to manage sponsor logos.",
    );
  }
 ...
 * hash means replacing a logo writes a new object instead of overwriting the
 * old one - no stale CDN copies, and nothing is destroyed before the database
 * has been updated.
 */
export async function uploadSponsorImage(input: {
  file: File;
  sponsorId: string;
  ...
  const path = `${safeSegment(input.sponsorId)}-${digest}.webp`;
  ...
  if (error) {
    return { ok: false, reason: "The logo could not be uploaded." };
  }
...
export async function deleteSponsorImage(path: string | null): Promise<void> {
  if (!path) return;
