...
function storageClient() {
  ...
  if (!url || !serviceRoleKey) {
    throw new Error(
      "SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must be set to manage panel photos.",
    );
  }
...
export async function uploadPanelImage(input: {
  ...
  const path = `${safeSegment(input.semester)}/${safeSegment(input.memberId)}-${digest}.webp`;
...
export async function deletePanelImage(path: string | null): Promise<void> {
  if (!path) return;
