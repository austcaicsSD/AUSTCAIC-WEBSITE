...
const IconSvg = ({ name, className }: { name: string, className?: string }) => {
  const icons: Record<string, React.ReactNode> = {
    ...
    sparkles: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
  };

  return <svg className={className || "w-6 h-6"} fill="none" stroke="currentColor" viewBox="0 0 24 24">{icons[name] || icons.target}</svg>;
};

// Gets Real Brand Logos for Tools
const getToolLogoUrl = (toolName: string) => {
  ...
  return map[toolName] ? `https://cdn.simpleicons.org/${map[toolName]}/white` : null;
};
...
const getTopicIcon = (topic: string, fallback: string) => {
  ...
  if (lower.includes("recon") || lower.includes("osint") || lower.includes("search") || lower.includes("find")) return "search";
  if (lower.includes("network") || lower.includes("tcp") || lower.includes("osi") || lower.includes("protocol")) return "network";
  if (lower.includes("linux") || lower.includes("cli") || lower.includes("terminal")) return "terminal";
  if (lower.includes("crypto") || lower.includes("hash") || lower.includes("password") || lower.includes("cipher") || lower.includes("security")) return "lock";
  if (lower.includes("vuln") || lower.includes("exploit") || lower.includes("xss") || lower.includes("injection") || lower.includes("bug") || lower.includes("hack")) return "bug";
  if (lower.includes("forensic") || lower.includes("steg") || lower.includes("metadata") || lower.includes("identity")) return "fingerprint";
  if (lower.includes("web") || lower.includes("http")) return "web";
  if (lower.includes("data") || lower.includes("eda") || lower.includes("visual") || lower.includes("scale")) return "chart";
  if (lower.includes("machine learning") || lower.includes("ml ") || lower.includes("model") || lower.includes("algorithm")) return "brain";
  if (lower.includes("deep learning") || lower.includes("neural") || lower.includes("cnn") || lower.includes("rnn") || lower.includes("ann")) return "nodes";
  if (lower.includes("python") || lower.includes("variable") || lower.includes("loop") || lower.includes("function") || lower.includes("code")) return "code";
  if (lower.includes("research") || lower.includes("paper") || lower.includes("literature") || lower.includes("publish") || lower.includes("ethics")) return "book";
  if (lower.includes("prize") || lower.includes("perform") || lower.includes("cert") || lower.includes("award") || lower.includes("career")) return "trophy";
  return fallback;
};

const getToolFallbackIcon = (tool: string) => {
  const t = tool.toLowerCase();
  if (t.includes("ctf") || t.includes("hack") || t.includes("exploit")) return "bug";
  if (t.includes("db") || t.includes("sql") || t.includes("sheet")) return "data";
  if (t.includes("cert") || t.includes("award") || t.includes("swag")) return "trophy";
  return "terminal";
};
...
export default function UltimateSyllabus() {
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 md:px-6 py-10 md:py-16 flex flex-col items-center min-h-screen justify-center">
          ...
          <div className="flex flex-col items-center mb-10 w-full animate-fade-in-up">
            ...
            <p className="text-sm md:text-lg lg:text-xl text-gray-400 text-center max-w-2xl font-medium leading-relaxed px-4 text-shadow-sm mt-2">
              Explore the future techs with <span className="text-white font-bold">AUSTCAIC</span>. The most rigorous, hands-on, and industry-focused curriculum.
            </p>
          </div>

          {/* HIGHLIGHTS ROW: Schedule & Prizes */}
          <div className="flex flex-wrap justify-center gap-3 md:gap-5 w-full max-w-5xl mb-12 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
             ...
             <div className="flex items-center gap-3 bg-[#0a0a0c]/80 border border-white/10 rounded-2xl px-5 py-3.5 backdrop-blur-xl shadow-lg hover:border-cyan-500/50 hover:bg-white/5 transition-all">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] md:text-xs text-gray-400 font-black uppercase tracking-widest">Cyber & Research</span>
                  <span className="text-xs md:text-sm font-bold text-white">Every <span className="text-cyan-400">Friday</span></span>
                </div>
             </div>
             ...
             <div className="flex items-center gap-3 bg-[#0a0a0c]/80 border border-white/10 rounded-2xl px-5 py-3.5 backdrop-blur-xl shadow-lg hover:border-purple-500/50 hover:bg-white/5 transition-all">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] md:text-xs text-gray-400 font-black uppercase tracking-widest">AI & ML</span>
                  <span className="text-xs md:text-sm font-bold text-white">Every <span className="text-purple-400">Saturday</span></span>
                </div>
             </div>
             ...
             <div className="flex items-center gap-3 bg-amber-500/10 border border-amber-500/30 rounded-2xl px-5 py-3.5 backdrop-blur-xl shadow-[0_0_20px_rgba(245,158,11,0.15)] relative overflow-hidden group cursor-default">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] md:text-xs text-amber-200/70 font-black uppercase tracking-widest">Rewards</span>
                  <span className="text-xs md:text-sm font-black text-amber-400 tracking-wide drop-shadow-md">Exclusive Prizes for Best Performers</span>
                </div>
             </div>
          </div>
          ...
          {/* 3 DOMAINS CARDS */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 md:gap-6 w-full animate-fade-in-up pb-8" style={{ animationDelay: '300ms' }}>
  ...
  const theme = 
    view === "cyber" ? { color: "cyan", hex: "text-cyan-400", bg: "bg-cyan-500", border: "border-cyan-500/50", shadow: "shadow-[0_0_40px_rgba(6,182,212,0.3)]", dot: "bg-cyan-400 shadow-[0_0_20px_#22d3ee]", glow: "group-hover:border-cyan-500/50", line: "from-cyan-500", iconBorder: "border-cyan-500/30", topicIcon: "terminal" } : 
    view === "aiml" ? { color: "purple", hex: "text-purple-400", bg: "bg-purple-500", border: "border-purple-500/50", shadow: "shadow-[0_0_40px_rgba(168,85,247,0.3)]", dot: "bg-purple-400 shadow-[0_0_20px_#c084fc]", glow: "group-hover:border-purple-500/50", line: "from-purple-500", iconBorder: "border-purple-500/30", topicIcon: "sparkles" } : 
    { color: "indigo", hex: "text-indigo-400", bg: "bg-indigo-500", border: "border-indigo-500/50", shadow: "shadow-[0_0_40px_rgba(99,102,241,0.3)]", dot: "bg-indigo-400 shadow-[0_0_20px_#818cf8]", glow: "group-hover:border-indigo-500/50", line: "from-indigo-500", iconBorder: "border-indigo-500/30", topicIcon: "checkCircle" };
    ...
    <main className={`relative min-h-screen bg-[#020202] font-sans text-gray-300 pb-28 pt-0 relative selection:bg-white/20 transition-opacity duration-400 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <div className="sticky top-0 z-50 w-full bg-[#020202]/70 backdrop-blur-2xl border-b border-white/10 shadow-2xl px-4 md:px-8 py-3 md:py-4 mb-6 md:mb-8 transition-all">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
          ...
          <button onClick={() => handleViewChange('landing')} className="group inline-flex items-center justify-center w-10 h-10 md:w-auto md:h-auto md:px-4 md:py-2 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 transition-all text-gray-400 hover:text-white active:scale-95 shrink-0 overflow-hidden relative">
          ...
          <div className="flex items-center gap-3">
             <div className={`w-9 h-9 md:w-12 md:h-12 ${theme.border} border bg-white/5 rounded-lg md:rounded-xl flex items-center justify-center ${theme.shadow} ${theme.hex}`}>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-8">
        ...
        <div className="relative border-l border-white/10 ml-4 md:ml-8 pb-6">
          ...
          {activeData.map((data, index) => {
            ...
            return (
              <div key={index} className="relative pl-6 md:pl-10 mb-4 md:mb-6 stagger-animate" style={{ animationDelay: `${index * 60}ms` }}>
                ...
                <div className="absolute -left-[6px] top-6 md:top-8 w-3 h-3 rounded-full bg-[#020202] flex items-center justify-center z-10 ring-4 ring-[#020202]">
                   <div className={`w-full h-full rounded-full transition-all duration-500 ${isActive ? (isFinale ? 'bg-amber-400 shadow-[0_0_15px_#fbbf24]' : theme.dot) : 'bg-gray-700'}`}></div>
                </div>

                {/* Interactive Holographic Card */}
                <div className={`group bg-[#0a0a0c]/80 rounded-[1rem] md:rounded-[1.5rem] border transition-all duration-500 overflow-hidden backdrop-blur-xl ${isActive ? (isFinale ? `border-amber-500/50 shadow-[0_0_40px_rgba(245,158,11,0.3)] bg-[#151005]` : `${theme.border} ${theme.shadow} bg-[#111115]`) : `border-white/5 hover:border-white/20 ${theme.glow}`}`}>
                  
                  {/* Card Header */}
                  <button onClick={() => setActiveWeek(isActive ? null : index)} className="w-full px-4 md:px-6 py-4 md:py-5 flex items-start md:items-center justify-between text-left focus:outline-none relative overflow-hidden active:bg-white/5 md:active:bg-transparent">
                    
                    {/* Active Scanline Highlight */}
                    {isActive && <div className={`absolute bottom-0 left-0 w-full h-[2px] ${isFinale ? 'bg-amber-500 shadow-[0_0_15px_#fbbf24]' : `${theme.bg} shadow-[0_0_15px_currentColor] ${theme.hex}`}`}></div>}
                    ...
                    <div className="flex flex-row items-center gap-4 w-full">
                      <div className={`w-10 h-10 md:w-14 md:h-14 shrink-0 rounded-lg md:rounded-xl flex items-center justify-center transition-all duration-500 border ${isActive ? (isFinale ? `border-amber-500/50 bg-amber-500 bg-opacity-20 text-amber-400` : `${theme.border} ${theme.bg} bg-opacity-20 ${theme.hex}`) : 'border-white/5 bg-white/5 text-gray-500 group-hover:text-gray-300'}`}>
                         <IconSvg name={data.icon} className="w-5 h-5 md:w-7 md:h-7" />
                      </div>
                      
                      <div className="flex flex-col flex-grow">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          <span className={`text-[10px] md:text-[11px] font-black uppercase tracking-widest ${isActive ? (isFinale ? 'text-amber-400' : theme.hex) : 'text-gray-500'}`}>{data.week}</span>
                          <span className="text-[10px] md:text-[11px] font-bold text-gray-400 bg-white/5 px-2 py-0.5 rounded border border-white/5 tracking-wider">{data.date}</span>
                        </div>
                        <h3 className={`font-bold text-base md:text-xl transition-colors duration-300 tracking-tight leading-tight ${isActive ? "text-white drop-shadow-md" : "text-gray-400 group-hover:text-white"}`}>
                          {data.title}
                        </h3>
                      </div>

                      <div className={`shrink-0 transition-transform duration-500 p-2 bg-white/5 rounded-lg border border-white/5 ${isActive ? `rotate-180 ${isFinale ? 'text-amber-400 border-amber-500/30' : `${theme.hex} ${theme.iconBorder}`}` : "text-gray-600"}`}>
                  ...
                  {/* Card Body Content */}
                  <div className={`transition-all duration-700 ease-in-out ${isActive ? "max-h-[2000px] opacity-100 pb-5 md:pb-6" : "max-h-0 opacity-0 pb-0"}`}>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-8">
                        ...
                        <div className="bg-black/30 rounded-xl md:rounded-2xl p-4 md:p-5 border border-white/5 shadow-inner">
                          <h4 className="text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-3 md:mb-4 flex items-center gap-2">
                             <div className={`w-6 h-6 rounded-md bg-white/5 flex items-center justify-center border border-white/5 ${isFinale ? 'text-amber-400' : theme.hex}`}>
                          ...
                          <ul className="space-y-2">
                            {data.topics.map((topic, tIndex) => {
                              const topicIcon = getTopicIcon(topic, theme.topicIcon);
                              return (
                                <li key={tIndex} className="flex items-start gap-3 p-1.5 md:p-2 rounded-lg hover:bg-white/5 border border-transparent hover:border-white/5 transition-all duration-300">
                                  <div className={`mt-0.5 shrink-0 p-1.5 rounded-lg bg-black border border-white/5 ${isFinale ? 'text-amber-400 shadow-[0_0_8px_#fbbf24]' : `${theme.hex} shadow-inner`}`}>
                                      <IconSvg name={topicIcon} className="w-3.5 h-3.5"/>
                                  </div>
                                  <span className="text-sm md:text-sm text-gray-300 font-medium leading-relaxed">{topic}</span>
                                </li>
                              )
                            })}
                          </ul>
                        ...
                        <div className="space-y-4 md:space-y-5">
                            ...
                            <div className="bg-black/30 rounded-xl md:rounded-2xl p-4 md:p-5 border border-white/5 shadow-inner">
                              ...
                              <ul className="space-y-2">
                                {data.outcomes.map((outcome, oIndex) => (
                                  <li key={oIndex} className="flex items-start gap-3 p-1">
                                    <svg className={`mt-0.5 shrink-0 w-4 h-4 ${isFinale ? 'text-amber-400' : theme.hex}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>
                          ...
                          {/* Tools Section */}
                          {data.tools && (
                            <div className={`bg-gradient-to-br ${isFinale ? 'from-amber-500/10' : 'from-white/5'} to-transparent rounded-xl md:rounded-2xl p-4 md:p-5 border ${isFinale ? 'border-amber-500/20' : 'border-white/10'} shadow-lg`}>
                               <h4 className="text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-3 md:mb-4 flex items-center gap-2">
                                 <div className={`w-6 h-6 rounded-md bg-white/5 flex items-center justify-center border border-white/5 ${isFinale ? 'text-amber-400' : theme.hex}`}>
                               ...
                               <div className="flex flex-wrap gap-2">
                                 {data.tools.map((tool, tlIndex) => {
                                   ...
                                   return (
                                     <span key={tlIndex} className={`flex items-center gap-2 px-3 py-1.5 bg-black border ${isFinale ? 'border-amber-500/30 text-amber-400 hover:border-amber-400' : `border-white/10 ${theme.hex} hover:border-white/30`} text-[11px] md:text-xs font-bold rounded-lg shadow-sm hover:-translate-y-0.5 transition-all cursor-default`}>
                                       {logoUrl ? (
                                         <img src={logoUrl} alt={tool} className="w-3.5 h-3.5" loading="lazy" />
                                       ) : (
                                         <IconSvg name={getToolFallbackIcon(tool)} className="w-3.5 h-3.5" />
                                       )}
                                       {tool}
                                     </span>
                                   );
